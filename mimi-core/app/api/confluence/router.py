...
def get_job_manager() -> ConfluenceJobManager:
...
def get_sync_worker() -> ConfluenceSyncWorker:
...
async def start_full_sync(
    ...
    try:
        ...
        jm = get_job_manager()
        sw = get_sync_worker()
        job_id = jm.create_job(request)
        
        # Start async execution
        asyncio.create_task(sw.execute_sync_job(jm, job_id))
        ...
        return ConfluenceFullSyncResponse(
            job_id=job_id,
            status=jm.get_job_status(job_id).status
        )
...
async def cancel_sync_job(
    ...
    try:
        jm = get_job_manager()
        success = jm.cancel_job(request.job_id)
...
async def get_sync_report(
    ...
    try:
        ...
        # Get report data
        report_data = jm.get_job_report(job_id, type)
