...
export const apiService = {
  ...
  async getDocuments(params?: { source?: string; limit?: number }): Promise<AdminDocsResponse> {
    const response = await api.get<AdminDocsResponse>('/admin/docs', { params });
    return response.data;
  },

  // Admin - list ingestion events
  async getUpdates(params?: { limit?: number }): Promise<AdminUpdatesResponse> {
    const response = await api.get<AdminUpdatesResponse>('/admin/updates', { params });
    return response.data;
  },
